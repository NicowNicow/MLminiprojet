...
// Added by Axel Spoerl, 22.07.2020
/// Intention: Add to_XML function

// Serialize Recurrent Layer
tinyxml2::XMLDocument* RecurrentLayer::to_XML() const
{
   ...
   tinyxml2::XMLElement* recurrent_element = doc->NewElement("RecurrentLayer");
   doc->InsertFirstChild(recurrent_element);

   // Read number of inputs and neurons
   const size_t n_inputs  = RecurrentLayer::get_inputs_number();
   const size_t n_neurons = RecurrentLayer::get_neurons_number();
   ...
   recurrent_element->InsertEndChild(inputs_element);
   ...
   recurrent_element->InsertEndChild(neurons_element);
   ...
   recurrent_element->InsertEndChild(activation_element);
   ...
   char * PARAMS = new char[xmltext.length() + 1];
   strcpy (PARAMS,xmltext.c_str());

   // Insert Parameter section
   tinyxml2::XMLElement* parameters_element = doc->NewElement("Parameters");
   recurrent_element->InsertEndChild(parameters_element);
   tinyxml2::XMLText* parameters_text = doc->NewText(PARAMS);
   parameters_element->InsertEndChild(parameters_text);
   recurrent_element->InsertEndChild(parameters_element);
   delete PARAMS;  // no longer needed

   // Finish the Perceptron layer section
   doc->InsertEndChild(recurrent_element);
   return doc;
}
