...
tinyxml2::XMLDocument* NeuralNetwork::to_XML() const
{
    ...
    Layer::LayerType ltype = layers_pointers[l]->get_type();
       ...
       switch (ltype) {
              ...
              {
                  // Start the LSTM section
                  // get the pointer and retrieve the subtree document
                  LongShortTermMemoryLayer* lstm_layer_pointer = get_long_short_term_memory_layer_pointer();
                  tinyxml2::XMLDocument* lstm_document = lstm_layer_pointer->to_XML();

                  // copy the subtree to the neural network object
                  tinyxml2::XMLElement* lstm_element = lstm_document->FirstChildElement("LongShortTermMemoryLayer");
                  tinyxml2::XMLNode* node = lstm_element->DeepClone(doc);
                  neural_network_element->InsertEndChild(node);

                  // Delete the recurrent document
                  delete lstm_document;
                  break;
              }
              case Layer::Recurrent:
              {
                  // Start the recurrent section
                  // get the pointer and retrieve the subtree document
                  RecurrentLayer* recurrent_layer_pointer = get_recurrent_layer_pointer();
                  tinyxml2::XMLDocument* recurrent_document = recurrent_layer_pointer->to_XML();
   
                  // copy the subtree to the neural network object
                  tinyxml2::XMLElement* recurrent_element = recurrent_document->FirstChildElement("RecurrentLayer");
                  tinyxml2::XMLNode* node = recurrent_element->DeepClone(doc);
                  neural_network_element->InsertEndChild(node);
   
                  // Delete the recurrent document
                  delete recurrent_document;
                  break;
              }
              case Layer::Scaling:
              {
    ...
    // Start the output section
    outputs2xml(doc, outputs_names, neural_network_element);
