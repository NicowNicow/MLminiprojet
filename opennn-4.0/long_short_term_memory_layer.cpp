...
// Added by Axel Spoerl, 29.07.2020
/// Intention: Add to_XML function

/// Returns a string serialisation of a vector with a given delimiter
/// @@TODO: Put this function into the right place and make it available for other layer types
string vector_to_string(vector<double> container, string deli)
{
   ...
   for (size_t i=0; i<container.size(); ++i){
     buffer = buffer + to_string(container[i]);
     if (i < (container.size() - 1 )) {buffer = buffer + deli;};
   }

   return buffer;
};

/// Converts a Tensor into a Matrix by serialising dimensions 2 & 3.
Matrix<double> tensor_to_matrix (Tensor<double> container_t) {

    // Get dimensions
    if (container_t.get_dimensions_number() != 3) {throw std::runtime_error( "We need 3 dimensions");}
    vector<size_t> dimensions   = container_t.get_dimensions();
    const size_t inputs_number  = dimensions[0];
    const size_t neurons_number = dimensions[1];
    const size_t types_number   = dimensions[2];

    // Tensor<double> weights(inputs_number,neurons_number,4);

    // reshape the tensor into 2d
    Matrix<double> container_m(inputs_number * neurons_number, types_number);
...
/// Help function to create an XML child series for a matrix
/// @@TODO: Put this function into the right place and make it available for other layer types
void matrix_to_XML (tinyxml2::XMLDocument* doc, tinyxml2::XMLElement* element, vector<string> XML_names, Matrix<double>& values) {

   // Loop over XML names
   for (size_t i=0; i<XML_names.size(); ++i){

       // Create a subchild per XML name
       string xmltext=XML_names[i];
       char * SUBCHILD = new char[xmltext.length() + 1];
       strcpy (SUBCHILD,xmltext.c_str());
       tinyxml2::XMLElement* subchild_element = doc->NewElement(SUBCHILD);
       element->InsertFirstChild(subchild_element);

       // Serialize the matrix into a char
       xmltext = vector_to_string(values.get_column(i), " ");
       char * VALUETEXT = new char[xmltext.length() + 1];
       strcpy (VALUETEXT,xmltext.c_str());

       // Insert the value and end the child
       tinyxml2::XMLText* value_text = doc->NewText(VALUETEXT);
       subchild_element->InsertEndChild(value_text);
       element->InsertEndChild(subchild_element);
       delete[] VALUETEXT;  // no longer needed
       delete[] SUBCHILD;  // no longer needed

   };

};

// Serialize LSTM Layer
tinyxml2::XMLDocument* LongShortTermMemoryLayer::to_XML() const
{
   ...
   tinyxml2::XMLElement* longshortterm_element = doc->NewElement("LongShortTermMemoryLayer");
   doc->InsertFirstChild(longshortterm_element);

   // Read number of inputs and neurons
   const size_t n_inputs  = LongShortTermMemoryLayer::get_inputs_number();
   const size_t n_neurons = LongShortTermMemoryLayer::get_neurons_number();

    // Insert and child for inputs number
   xmltext = to_string(n_inputs);
   char * INP_NUM = new char[xmltext.length() + 1];
   strcpy (INP_NUM,xmltext.c_str());
   tinyxml2::XMLElement* inputs_element = doc->NewElement("InputsNumber");
   longshortterm_element->InsertFirstChild(inputs_element);
   tinyxml2::XMLText* inputs_number_text = doc->NewText(INP_NUM);
   inputs_element->InsertEndChild(inputs_number_text);
   longshortterm_element->InsertEndChild(inputs_element);
   delete[] INP_NUM;  // no longer needed

   // Insert and child for neurons number
   xmltext = to_string(n_neurons);
   char * NEU_NUM = new char[xmltext.length() + 1];
   strcpy (NEU_NUM,xmltext.c_str());
   tinyxml2::XMLElement* neurons_element = doc->NewElement("NeuronsNumber");
   longshortterm_element->InsertFirstChild(neurons_element);
   tinyxml2::XMLText* neurons_number_text = doc->NewText(NEU_NUM);
   neurons_element->InsertEndChild(neurons_number_text);
   longshortterm_element->InsertEndChild(neurons_element);
   delete[] NEU_NUM;  // no longer needed

   // Insert and end child for activation function
   xmltext = write_activation_function();
   char * ACTIVATION_FUN = new char[xmltext.length() + 1];
   strcpy (ACTIVATION_FUN,xmltext.c_str());
   tinyxml2::XMLElement* activation_element = doc->NewElement("ActivationFunction");
   longshortterm_element->InsertFirstChild(activation_element);
   tinyxml2::XMLText* activation_text = doc->NewText(ACTIVATION_FUN);
   activation_element->InsertEndChild(activation_text);
   longshortterm_element->InsertEndChild(activation_element);
   delete[] ACTIVATION_FUN;  // no longer needed

   // Insert main child for biases
   tinyxml2::XMLElement* biases_element = doc->NewElement("Biases");
   longshortterm_element->InsertFirstChild(biases_element);

   // Read all biases
   Matrix<double> biases = get_biases();

   // Bias container is hard coded:
   // 0= forget biases
   // 1= input biases
   // 2= state biases
   // 3= output biases

   // Vector for XML Names
   vector<string> biasnames = {"ForgetBiases", "InputBiases", "StateBiases", "OutputBiases"};

   // Create the subchildren
   matrix_to_XML(doc, biases_element, biasnames, biases);

   // end the bias child
   longshortterm_element->InsertEndChild(biases_element);

   // Get weights
   Tensor<double> weights = get_weights();

   // wight types are hard coded
   // 0= forget_weights
   // 1= input_weights
   // 2= state_weights
   // 3= output_weights

   // Insert main child for weights
   tinyxml2::XMLElement* weights_element = doc->NewElement("Weights");
   longshortterm_element->InsertFirstChild(weights_element);

   // Tensor<double> weights(inputs_number,neurons_number,4);
   // Serialise each value per neuron and input into matrix
   Matrix<double> container_m = tensor_to_matrix(weights);
   vector<string> weightnames = {"ForgetWeights", "InputWeights", "StateWeights", "OutputWeights"};

   // Create the subchildren
   matrix_to_XML(doc, weights_element, weightnames, container_m);

   // end the weight child
   longshortterm_element->InsertEndChild(weights_element);
